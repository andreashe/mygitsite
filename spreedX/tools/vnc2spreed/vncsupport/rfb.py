...
##  pyvnc2swf - rfb.py
##
##  $Id: rfb.py,v 1.1 2007/03/31 04:02:16 euske Exp $
##
...
from crippled_des import DesCipher, decrypt_passwd
...
class RFBProxy:
  ...
  def change_size(self, width, height):
    ...
    if self.fb:
        self.fb.change_size(width, height)
    ...
    if c == '':
      ...
      for rectindex in xrange(nrects):
        ...
        # Size changed
          ...
          if self.debug:
            print >>stderr, 'ChangedSize: %d,%d' % (width,height)
...
class RFBNetworkClient(RFBProxy):
  ...
  def request_update(self, incremental=True):
    if self.debug:
      ...
      print >>stderr, 'incremental %s' % incremental
    if incremental:
        incremental = '\x01'
    else:
        incremental = '\x00'
    self.send('\x03%s' % (incremental) + pack('>HHHH', *self.clipping))
    return
...
class RFBConverter(RFBFrameBuffer):
  ...
  def change_size(self, width, height):
    self.info.set_size(width, height)
    return
...
class RFBStreamConverter(RFBConverter):
  ...
  def change_size(self, width, height):
    RFBConverter.change_size(self, width, height)
    self.stream.change_size(width, height)
